...
public class CascadedShadowMapping : MonoBehaviour
{
    ...
    private void CreateRenderTexture()
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            depthTextures[i] = new RenderTexture(256, 256, 24, rtFormat);
    ...
    void CalcLightCameraSplitsFrustum()
    {
        ...
        for (int k = 0; k < 4; k++)
        {
            ...
            float wDis = Vector3.Magnitude(lightCamera_Splits_fcs[k].nearCorners[1] -
                lightCamera_Splits_fcs[k].nearCorners[0]);
            float hDis = Vector3.Magnitude(lightCamera_Splits_fcs[k].nearCorners[3] -
                lightCamera_Splits_fcs[k].nearCorners[0]);
            float crossDis =
    ...
    void ConstructLightCameraSplits(int k)
    {
        ...
        dirLightCamera.aspect = 1;
        
        float wDis = Vector3.Magnitude(lightCamera_Splits_fcs[k].nearCorners[0] -
                                       lightCamera_Splits_fcs[k].nearCorners[1]);
        float hDis = Vector3.Magnitude(lightCamera_Splits_fcs[k].nearCorners[0] -
                                       lightCamera_Splits_fcs[k].nearCorners[3]);
        ...
        dirLightCamera.orthographicSize = maxDis * 0.5f;
    }
